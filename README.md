...
Додаток: "Шпаргалка" Команд

Примітка: Всі команди perf використовують повний шлях до бінарного файлу, який ми знайшли під час роботи: /usr/lib/linux-tools/6.8.0-87-generic/perf

5.1. Компіляція

Bash

# Неоптимізована версія (з дебаг-символами)
g++ -O0 -g primes.cpp -o primes_O0

# Оптимізована версія (з дебаг-символами)
g++ -O3 -g primes.cpp -o primes_O3

5.2. Побудова FlameGraph (п. 1.1)

(Замініть primes_O0 на primes_O3 та out.perf на out_O3.perf для оптимізованої версії)
Bash

# 1. Запис даних профілювання
sudo /usr/lib/linux-tools/6.8.0-87-generic/perf record -F 99 -g ./primes_O0 300000

# 2. Конвертація у текстовий формат
sudo /usr/lib/linux-tools/6.8.0-87-generic/perf script > out.perf

# 3. Згортання стеків (скрипт з папки FlameGraph)
FlameGraph/stackcollapse-perf.pl out.perf > out.folded

# 4. Генерація SVG
FlameGraph/flamegraph.pl out.folded > primes_O0.svg

5.3. Збір Статистики (п. 2)

(Замініть primes_O0 на primes_O3 для оптимізованої версії)
Bash

# Утиліта time (загальний час, User/System)
/usr/bin/time --verbose ./primes_O0 300000

# perf stat (детальна апаратна статистика: інструкції, цикли, IPC, кеш)
sudo /usr/lib/linux-tools/6.8.0-87-generic/perf stat -d ./primes_O0 300000

# perf report (інтерактивна текстова версія FlameGraph)
sudo /usr/lib/linux-tools/6.8.0-87-generic/perf report

5.4. Вимірювання Енерговитрат (п. 3)

(Замініть primes_O0 на primes_O3 для оптимізованої версії)
Bash

# Перевірка, чи підтримує CPU лічильники RAPL
sudo /usr/lib/linux-tools/6.8.0-87-generic/perf list | grep 'power/energy'

# Вимірювання споживання в Джоулях (тільки для програми)
sudo /usr/lib/linux-tools/6.8.0-87-generic/perf stat -e 'power/energy-pkg/' -e 'power/energy-cores/' ./primes_O0 300000

5.5. Керування Частотою CPU (для "Еко-Тесту")

Bash

# 1. Перевірка поточного стану CPU
cpupower frequency-info

# 2. ВСТАНОВЛЕННЯ ОБМЕЖЕННЯ (Еко-режим)
sudo cpupower frequency-set --max 1.5G

# 3. ЗНЯТТЯ ОБМЕЖЕННЯ (Повернення до норми)
sudo cpupower frequency-set --max 4.7G
